...
const Hexagon: React.FC<HexagonProps> = ({
  ...
  const isSpecial =
    hexId === '2-1' || hexId === 'R4-1' || hexId === '5-1' || hexId === 'R1-1'
  ...
  const getNeighborGlow = () => {
    const activeSpecialHexes = Array.from(activeHexagons.keys()).filter(
      (id) =>
        (id === '2-1' || id === 'R4-1' || id === '5-1' || id === 'R1-1') &&
        activeHexagons.get(id)! > 0
    )
    ...
    activeSpecialHexes.forEach((specialId) => {
      const [specialRow, specialCol] = specialId
        .replace('R', '')
        .split('-')
        .map(Number)
      const distance = Math.sqrt(
        Math.pow(row - specialRow, 2) + Math.pow(col - specialCol, 2)
      )
  ...
  const hexPath = useMemo(() => {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      const cornerStart = [round(current[0] - ux1), round(current[1] - uy1)]
      const cornerEnd = [round(current[0] + ux2), round(current[1] + uy2)]
      ...
      path += `Q ${round(current[0])} ${round(current[1])} ${cornerEnd[0]} ${cornerEnd[1]} `
    }
  ...
  const renderDots = () => {
    ...
    const isInsideHexagon = (x: number, y: number, radius: number) => {
      ...
      for (let i = 0, j = hexPoints.length - 1; i < hexPoints.length; j = i++) {
        const xi = hexPoints[i][0],
          yi = hexPoints[i][1]
        const xj = hexPoints[j][0],
          yj = hexPoints[j][1]
          ...
          <>
            ...
            <g mask={`url(#${blurInfo.direction}Mask)`}>
              <path
                ...
                style={{
                  transition:
                    'stroke 1.5s ease-out, stroke-width 0.3s ease-out',
...
export const HexPattern: React.FC<HexPatternProps> = ({
  ...
  const [activeHexagons, setActiveHexagons] = useState<Map<string, number>>(
    new Map()
  )
  ...
  useEffect(() => {
    ...
    const fullCycleTime =
      trailSequence.length * 2 * animationDelay +
      pauseBetweenCycles +
      pauseBetweenFullCycles
  ...
  return (
    ...
    >
      <defs>
        ...
        <linearGradient
          id="topLeftToBottomRightGradient"
          x1="0%"
          y1="0%"
          x2="100%"
          y2="100%"
